...
| `TB_AI_MONTHLY_LIMIT` | No | Managed-AI monthly quota: the maximum number of AI-consuming operations (search + capture + note ingest) permitted per UTC calendar month, counted from `function_call_logs`. **Unset, empty, non-numeric, or ≤ 0 means unlimited** — the default, correct for self-hosters who bring their own `OPENROUTER_API_KEY`; a misconfiguration never blocks all AI. When set to a positive integer, an over-quota AI operation is refused **before any AI call** with a distinct quota-exceeded error (MCP tools) / `429` (the `ingest-note` route) — never a silent empty result. Metered set: `search_thoughts`, `capture_thought`, `ingest-note`; other AI-consuming surfaces (`write_document`/`update_document`, `update_thought`'s content path) are not metered in this version. Best-effort cost control, not a security boundary — a transient metering-query failure fails open (logged). |
