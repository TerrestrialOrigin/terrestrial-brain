#!/usr/bin/env bash
# Validate the whole repo: Deno integration suite + plugin tests + plugin build.
...
echo "==> Running Deno test suite (integration)..."
(cd "$REPO_ROOT" && deno test --allow-net --allow-env tests/)
