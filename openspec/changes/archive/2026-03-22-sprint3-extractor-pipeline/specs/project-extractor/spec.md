...
- **WHEN** a parsed note has `referenceId` of `projects/Test Proj/sprint-notes.md`
- **AND** a project named "Test Proj" exists in the database
- **THEN** the extractor SHALL return the Test Proj project's ID in `ids`

#### Scenario: Note in a nested project path
- **WHEN** a parsed note has `referenceId` of `projects/Test Proj/sprints/week1.md`
- **AND** a project named "Test Proj" exists
- **THEN** the extractor SHALL return Test Proj's ID (matching on the first segment after `projects/`)
...
- **WHEN** a parsed note has `referenceId` of `projects/DemoProj/kickoff.md`
- **AND** no project named "DemoProj" exists
- **THEN** the extractor SHALL create a project row with `name: "DemoProj"` and return its ID

#### Scenario: Auto-created project enriches context
- **WHEN** the ProjectExtractor auto-creates a project "DemoProj"
...
- **WHEN** a parsed note has a heading `# Test Proj` or `## Test Proj`
- **AND** a project named "Test Proj" exists
...
- **WHEN** a parsed note has a heading `# test proj`
- **AND** a project named "Test Proj" exists
...
- **WHEN** a note's body text mentions "Test Proj" but has no matching heading or file path
...
- **WHEN** a note is at `projects/Test Proj/notes.md` AND has a heading `# Test Proj`
