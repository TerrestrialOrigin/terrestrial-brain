...
- **WHEN** the 8 integration test files are searched for `function callTool`, `function callToolRaw`, or `function callHTTP` definitions
...
The suite SHALL NOT contain placeholder test cases whose only assertion is `assertEquals(true, true)`. Cleanup logic SHALL live in `try/finally` blocks attached to the tests that own the fixtures, not in standalone pseudo-tests that inflate the pass count.
...
Pure, deterministic unit tests (no database, no network, no LLM) SHALL live under `tests/unit/`, and tests that exercise the live Supabase stack SHALL live under `tests/integration/`. The pure `parse.test.ts` and the non-DB portion of `extractors.test.ts` SHALL be moved to `tests/unit/`. The stray in-source test at `supabase/functions/terrestrial-brain-mcp/extractors/project-extractor.test.ts` SHALL be relocated into `tests/unit/`, with its `(globalThis as any).Deno` vitest shim rewritten as a plain Deno test.

#### Scenario: Unit tests run without the stack

- **WHEN** `deno task test:unit` is run with no Supabase stack and no network access to it
- **THEN** the unit tests under `tests/unit/` pass on their own (they perform no DB/network calls)

#### Scenario: No test file lives in the source tree
