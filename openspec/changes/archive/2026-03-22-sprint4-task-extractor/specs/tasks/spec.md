...
- **WHEN** the TaskExtractor detects a `- [ ] Fix the navbar` checkbox in a note with `referenceId` of `projects/Test Proj/sprint.md`
- **THEN** a new task row SHALL be inserted with `content: "Fix the navbar"`, `status: "open"`, `reference_id: "projects/Test Proj/sprint.md"`
...
- **WHEN** a note with `referenceId` of `projects/Test Proj/sprint.md` is re-ingested
- **THEN** the TaskExtractor SHALL query existing tasks WHERE `reference_id = 'projects/Test Proj/sprint.md'` to reconcile against
