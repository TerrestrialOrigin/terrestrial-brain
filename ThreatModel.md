...
| T16 | Mutation-ruleset bypass: a future memory console or PMS connector writes memory directly, skipping the one server-side update path, evading re-embed/re-hash and the dedup gate (stale search + duplicate leak — the disease the product cures) | **Specified** (memory-lifecycle-rules-spec) | The `memory-lifecycle-rules` delta spec makes the single-ruleset + INVARIANT-1 re-embed/re-hash guarantees `test`-tagged scenarios binding on *every* actor (LLM \| user \| sync); no console-only/connector-only write is authorized. Enforcement lands in Step 7 against these scenarios; recorded here as the spec-integrity threat this ruleset owns |
| T17 | Poisoned extraction: a hallucinated or injected `type` (or other metadata) flows unvalidated into a stored mutation | **Specified** (memory-lifecycle-rules-spec) | The spec requires `extractMetadata` output be *parsed* against the `THOUGHT_TYPES` allowlist (extended with `instruction`/`decision`) with an out-of-allowlist value coerced to the `observation` fallback and logged — `parse, don't cast` at the seam. `test`-tagged; implemented in Step 7 |
...
| T19 | Destructive supersession: contradiction handling that deletes/overwrites the older belief loses the audit trail and is unrecoverable | **Specified** (memory-lifecycle-rules-spec) | The spec requires supersession via a retained `supersedes` edge (older row kept, excluded from default retrieval, reversible via the resolve tool) — never delete/overwrite; `test`-tagged on the effect |
