...
| T12 | Destructive erasure abuse via `forget_note` / `/forget-note` (permanent hard-delete of a note's snapshot + thoughts) | **Mitigated** (gdpr-data-lifecycle) | Same `x-brain-key` gate as every other route (401 without it); scoped to a single `reference_id` — no bulk/wildcard delete; idempotent, so a replay does no extra damage; never reachable from an LLM path (contrast the LLM reconciliation path, which can only soft-archive per fix-plan Step 4) |
