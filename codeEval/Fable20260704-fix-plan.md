...
- Slack ingest: `supabase/functions/ingest-thought/index.ts`
...
### Step 2: Slack request-signature verification on ingest-thought
**Findings:** S2 · **Branch:** `bug/SlackSignatureVerification` · **Size:** S–M · **Depends on:** nothing

- In `supabase/functions/ingest-thought/index.ts`, verify `X-Slack-Signature` (HMAC-SHA256 with a new `SLACK_SIGNING_SECRET` env var) and reject `X-Slack-Request-Timestamp` older than 5 minutes, BEFORE any event processing. Keep the URL-verification challenge path working.
- Fail fast at cold start if `SLACK_SIGNING_SECRET` is unset (aligned with Step 10's approach; a local `requireEnv` inline is fine for now).
- Replace the content-based dedup (`ingest-thought/index.ts:165-179` — "same text within 5 min" drops legitimate repeats) with idempotency on Slack's `event_id` (or channel+`event.ts`), which exists precisely for retry dedup — store seen IDs in a small table or in `function_call_logs`.
- Update `scripts/initial-setup-prod.sh` secrets list and README env-var table.
- **Tests (failing first):** integration tests posting a well-formed Slack event WITHOUT a valid signature → 401; with a valid signature (compute HMAC in the test) → accepted; stale timestamp → rejected; same `event_id` retried → deduped; identical TEXT with a new `event_id` → ingested (fails against current content-based dedup). `tests/integration/ingest_note_route.test.ts` is the model for HTTP-level tests.
...
- Delete `logFunctionError` dead code in `ingest-thought/index.ts:39-51` and fix its inaccurate "fire-and-forget" banner while touching LLM plumbing.
...
- Fix the JSON-RPC positional coupling nit in `ingest-thought` (`result.id === 2` → named constant).
...
1. **Phase A first** because S1/S2 are exploitable-now security holes and C2/C3 destroy user data — all four steps are small and independent, so nothing justifies delaying them behind refactors.
...
- [ ] 2. Slack signature verification (`bug/SlackSignatureVerification`)
- [ ] 3. Header-based API auth (`feature/HeaderBasedAuth`)
- [ ] 4. Soft-archive thought deletes (`bug/SoftArchiveThoughtDeletes`)
- [ ] 5. Test suite foundation (`feature/TestSuiteFoundation`)
- [ ] 6. Atomic task creation (`bug/AtomicTaskCreation`)
- [ ] 7. Name-matching word boundaries (`bug/NameMatchingWordBoundary`)
- [ ] 8. Task-extractor merge semantics (`bug/TaskExtractorMergeSemantics`)
- [ ] 9. Date/timezone handling (`bug/DateTimezoneHandling`)
- [ ] 10. Fail-fast env & error surfacing (`bug/FailFastEnvAndErrors`)
- [ ] 11. Request-context isolation (`bug/RequestContextIsolation`)
- [ ] 12. Plugin sync-reliability bugs (`bug/PluginSyncReliability`)
- [ ] 13. Prod scripts hardening (`bug/ProdScriptsHardening`)
- [ ] 14. MCP envelope & logging refactor (`feature/McpEnvelopeRefactor`)
- [ ] 15. AI provider interface (`feature/AiProviderInterface`)
- [ ] 16. Repository layer — thoughts & tasks (`feature/RepositoryLayerCore`)
- [ ] 17. Repository layer — remaining entities (`feature/RepositoryLayerRemaining`)
- [ ] 18. Decompose server god-functions (`feature/DecomposeServerHandlers`)
- [ ] 19. Decompose task extractor (`feature/DecomposeTaskExtractor`)
- [ ] 20. Pipeline contracts & parser robustness (`feature/ExtractorPipelineContracts`)
- [ ] 21. Plugin modularization (`feature/PluginModularization`)
- [ ] 22. LLM stub mode (`feature/LlmStubMode`)
- [ ] 23. CI pipeline & one-command dev (`feature/CiPipeline`)
- [ ] 24. DB types & input validation (`feature/DbTypesAndValidation`)
- [ ] 25. GDPR data lifecycle (`feature/GdprDataLifecycle`)
