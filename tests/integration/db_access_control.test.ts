...
function restHeaders(apiKey: string, extra: Record<string, string> = {}): Record<string, string> {
  return {
    "apikey": apiKey,
...
async function serviceInsert(table: string, row: Record<string, unknown>): Promise<Record<string, unknown>> {
  const response = await fetch(`${REST_URL}/${table}`, {
    method: "POST",
    headers: restHeaders(SUPABASE_SERVICE_KEY, { "Prefer": "return=representation" }),
    body: JSON.stringify(row),
  });
  const body = await response.json();
  if (!response.ok) {
    throw new Error(`Service-role insert into ${table} failed: ${JSON.stringify(body)}`);
  }
  return body[0];
}

async function serviceSelectById(table: string, id: string): Promise<Record<string, unknown>[]> {
  const response = await fetch(`${REST_URL}/${table}?id=eq.${id}`, {
    headers: restHeaders(SUPABASE_SERVICE_KEY),
  });
  const body = await response.json();
  if (!response.ok) {
    throw new Error(`Service-role select from ${table} failed: ${JSON.stringify(body)}`);
  }
  return body;
}
...
Deno.test("anon key cannot read people", async () => {
  ...
  try {
    const anonResponse = await fetch(`${REST_URL}/people?id=eq.${seededPerson.id}`, {
    ...
    assertEquals(
      ...
      true,
      `anon select must be rejected, got ${anonResponse.status}: ${JSON.stringify(anonBody)}`,
    );
    assertEquals(anonBody.code, "42501", "rejection must be a permission-denied error");
  } finally {
...
Deno.test("anon key cannot insert into people", async () => {
  ...
  const anonResponse = await fetch(`${REST_URL}/people`, {
    method: "POST",
    headers: restHeaders(SUPABASE_ANON_KEY, { "Prefer": "return=representation" }),
  ...
  try {
    assertEquals(
      anonResponse.status === 401 || anonResponse.status === 403,
      true,
      `anon insert must be rejected, got ${anonResponse.status}: ${JSON.stringify(anonBody)}`,
    );
    assertEquals(anonBody.code, "42501", "rejection must be a row-level security violation");
    ...
    assertEquals(serviceCheckRows.length, 0, "no row may be created by an anon insert");
  } finally {
...
Deno.test("anon key cannot update people", async () => {
  ...
  try {
    ...
    assertEquals(
      anonResponse.status === 401 || anonResponse.status === 403,
      true,
      `anon update must be rejected, got ${anonResponse.status}: ${JSON.stringify(anonBody)}`,
    );
    const rowsAfter = await serviceSelectById("people", seededPerson.id as string);
    assertEquals(rowsAfter[0].email, originalEmail, "person data must be unchanged");
  } finally {
    await serviceDeleteById("people", seededPerson.id as string);
  }
});
...
Deno.test("anon key cannot delete from people", async () => {
  ...
  try {
    ...
    assertEquals(
      anonResponse.status === 401 || anonResponse.status === 403,
      true,
      `anon delete must be rejected, got ${anonResponse.status}: ${JSON.stringify(anonBody)}`,
    );
    const rowsAfter = await serviceSelectById("people", seededPerson.id as string);
    assertEquals(rowsAfter.length, 1, "person row must still exist after anon delete attempt");
  } finally {
    await serviceDeleteById("people", seededPerson.id as string);
  }
});
...
Deno.test("service role retains full CRUD on people", async () => {
  ...
  try {
    ...
    const updateResponse = await fetch(`${REST_URL}/people?id=eq.${seededPerson.id}`, {
      method: "PATCH",
      headers: restHeaders(SUPABASE_SERVICE_KEY, { "Prefer": "return=representation" }),
    ...
    assertEquals(updatedRows.length, 1, "service-role update must still affect the row");
    assertEquals(updatedRows[0].description, "updated by regression test");

    const selectedRows = await serviceSelectById("people", seededPerson.id as string);
    assertEquals(selectedRows.length, 1, "service-role select must still see the row");
  } finally {
    await serviceDeleteById("people", seededPerson.id as string);
  }
  const rowsAfterDelete = await serviceSelectById("people", seededPerson.id as string);
  assertEquals(rowsAfterDelete.length, 0, "service-role delete must still remove the row");
});
...
Deno.test("anon key cannot execute increment_usefulness", async () => {
  ...
  try {
    ...
    const anonBody = await anonResponse.json();
    assertEquals(
      anonResponse.status === 401 || anonResponse.status === 403 || anonResponse.status === 404,
      true,
      `anon RPC must be rejected, got ${anonResponse.status}: ${JSON.stringify(anonBody)}`,
    );

    const rowsAfter = await serviceSelectById("thoughts", seededThought.id as string);
    assertEquals(
...
Deno.test("service role can still execute increment_usefulness", async () => {
  ...
  try {
    ...
    const serviceRpcResponse = await fetch(`${REST_URL}/rpc/increment_usefulness`, {
      method: "POST",
      headers: restHeaders(SUPABASE_SERVICE_KEY),
      body: JSON.stringify({ thought_ids: [seededThought.id] }),
    });
    const affectedCount = await serviceRpcResponse.json();
    assertEquals(serviceRpcResponse.ok, true, `service-role RPC failed: ${JSON.stringify(affectedCount)}`);
