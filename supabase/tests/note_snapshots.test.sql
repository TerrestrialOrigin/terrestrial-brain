...
VALUES ('dddddddd-0000-0000-0000-000000000001', 'projects/Test Proj/planning.md', 'Planning', 'Original content', 'obsidian');

SELECT is(
  (SELECT count(*)::int FROM public.note_snapshots WHERE reference_id = 'projects/Test Proj/planning.md'),
  1,
  'Initial insert creates one row'
);

INSERT INTO public.note_snapshots (reference_id, title, content, source)
VALUES ('projects/Test Proj/planning.md', 'Planning v2', 'Updated content', 'obsidian')
...
SELECT is(
  (SELECT content FROM public.note_snapshots WHERE reference_id = 'projects/Test Proj/planning.md'),
  'Updated content',
  'Upsert updated the content to new value'
);

SELECT is(
  (SELECT title FROM public.note_snapshots WHERE reference_id = 'projects/Test Proj/planning.md'),
  'Planning v2',
...
SELECT throws_ok(
  $$INSERT INTO public.note_snapshots (reference_id, content) VALUES ('projects/Test Proj/planning.md', 'Duplicate ref')$$,
  '23505',
  NULL,
